%% ★ real 판 --- USV_swarm_defense_EAAI_KR.tex 와 같은 형식·같은 수식이되 도판만
%%   figs_snak/real/fig1..fig19.png 을 번호 순서대로 쓴다. 본문은 sections_real/ 에서 읽는다.
%%   원본(sections/, USV_swarm_defense_EAAI_KR.tex)은 손대지 않는다.
% !TEX program = xelatex
%==============================================================================
...
\maketitle
\input{sections_real/00_abstract}
\input{sections_real/00d_keywords_legacy}
...
\vspace{8pt}
\end{@twocolumnfalse}]
\input{sections_real/01_intro}
\input{sections_real/02_related}
\input{sections_real/03_problem}
\input{sections_real/04_method}
\input{sections_real/05_experiment}
\input{sections_real/06_results}
\input{sections_real/07_discussion}
\input{sections_real/08_limitations}
\input{sections_real/09_conclusion}
\input{sections_real/10_declarations}
\input{sections_real/refs}
% 영문 초록은 국내 논문 관례대로 맨 뒤.
\input{sections_real/00b_abstract_en}
