...
% 기호표. 본문 인용은 §7 뿐이라 위치가 자유롭다. §3 끝에 두면 §4 의 FloatBarrier 가
% 표를 다음 쪽으로 흘려보내며 p.4 나머지를 비우므로, §4 안에서 [!b] 로 선언해 같은 쪽
% 하단에 앉힌다(dblfloatfix).
%-------------------------------------------------- 기호표
...
\begin{figure*}[!tp]
\centering
\includegraphics[width=0.62\textwidth]{fig7.png}
...
육지 채널은 OpenStreetMap 해안선·섬 폴리곤을 래스터화한 실해역 마스크이며, 충돌 판정은 고해상도 물리 격자에서 이진으로, 관측 채널은 이를
면적비로 다운샘플한 연속값으로 쓴다.
...
둘째 줄은 두 점 사이 간격의 상·하한이다 --- $r_{\mathrm{dup}} = 200\unit{m}$
...
방어정 속력 $v_a$, 창 안 총 이동거리 $D$, 소모 그물 수 $N_{\mathrm{net}}$ 에 대해
\begin{equationfn}
\begin{aligned}
...
\label{eq:rdense}
\end{equationfn}
이다. $S_p \in [0,1]$ 은 창 안 누적 선회량을 최대 가능 선회량 $H\dot\psi_{\max}$ 로 나눈 값,
$\Xi_p$ 는 직전 결정 대비 목표점 이동을 $L_{\mathrm{net}}$ 으로 나눈 값이다. 근접 페널티는
창 안에서 방어정 $p$ 가 모선·타 방어정에 가장 가까웠던 거리 $d^{\mathrm{m}}_p$,
$d^{\mathrm{a}}_p$ 를 영향 반경으로 정규화한 뒤 제곱하여 붙을수록 급격히 커지게 한다.
\begin{equationsmall}
...
커버리지는 적에서 모선으로 향하는 직선(ray)이 설치된 그물 영역 $\mathcal{W}$ 에 막힌 생존
적의 비율로, 창 안에 포획이 없을 때도 후보를 변별하는 선행 지표다.
\begin{equationsmall}
...
\label{eq:coverage}
\end{equationsmall}
$\Lambda$ 는 $[0.05, 0.95]$ 를 16등분한 표본점이고, $\mathrm{Ray}$ 는 같은 검사를 계획된
그물벽까지 포함하되 적에 가까운 차단을 우대한 배율판이다($\mathrm{Ray} \in [0, 2.6]$).
가중은 $w_{\mathrm{path}} = 0.5$, $w_{\mathrm{net}} = 0.3$, $w_{\mathrm{turn}} = w_{\mathrm{cons}} = 0.3$,
$w_{\mathrm{prox}} = 0.75$, $w_{\mathrm{cov}} = 0.1$, $w_{\mathrm{ray}} = 1.6$ 이다. 커버리지를
제외한 항은 이벤트 계수의 수십분의 일 규모이고, 커버리지 항은 최대 $4.2$ 로 포획 1건($13.5$)의
3분의 1을 넘지 않게 두었다.
...
$i^{*}_k$ 를 중심으로 유효 픽셀 위에 정규화한 가우시안 가중이다 --- 인접 픽셀은 사실상 같은
행동이라 원-핫은 기울기가 지나치게 희소하기 때문이다.
\begin{equation}
...
배정은 코드가 보정하지 않고 그대로 쓰이므로 --- 비워 둔 \texttt{ally\_ids} 는 ``이 무리에는
아무도 보내지 않는다''를 뜻하여 그대로 무방비를 낳고(프롬프트가 이 대가를 명시한다), 지목되지
않은 방어정은 대기한다 --- \S\ref{sec:results}의 조건 간 차이는 배정 판단 자체에 귀속된다.
