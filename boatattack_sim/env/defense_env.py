...
class DefenseVecEnv:
    def __init__(self, num_worlds: int = 512, cfg: SimConfig = DEFAULT_CONFIG,
        ...
        # ── ★ 육지(섬) 장애물: 지형은 정적이라 __init__ 에서 1회 계산 후 전 에피소드 공유 ──
        #   단 **사이트(해역)는 여러 곳**이다(cfg.land_sites). 지형 일반화를 위해 월드마다
        #   사이트를 균등 추출하므로 모든 지형 배열은 선두에 사이트 축 S 를 갖는다.
        #   조회는 `arr[self.world_site[:, None], i, j]` 처럼 월드축과 함께 인덱싱한다.
        #     land_grid [S,G,G] : 물리격자 (충돌·적 APF 조회용)
        #     land_map  [S,n,n] : 점수맵 (관측 채널 + 행동 마스크용)
        #     land_dist/land_away [S,G,G],[S,G,G,2] : 적 APF 척력장
        from . import terrain as TR
        self.sites = TR.resolve_sites(cfg)                 # [(name, lat, lon)]
        self.S = len(self.sites)
        self.land_grid = TR.land_masks(cfg, self.G, self.sites)
        self.site_has_land = self.land_grid.any(axis=(1, 2))          # [S]
        self.has_land = bool(self.site_has_land.any())
        if getattr(cfg, "cnn_action", False):
            from . import cnn_map as _CM
            self.land_map = TR.land_masks(cfg, _CM.grid_n(cfg), self.sites)
        else:
            self.land_map = np.zeros((self.S, 1, 1), bool)
        if self.has_land and bool(getattr(cfg, "enemy_land_avoid", True)):
            self.land_dist, self.land_away = TR.repulsion_fields(cfg, self.G, self.sites)
        else:
            self.land_dist, self.land_away = None, None
        self.rng = np.random.default_rng(seed)
        # 월드별 사이트. reset() 전에 존재해야 한다(_spawn_worlds 가 첫 줄에서 다시 뽑는다).
        self.world_site = np.zeros(self.N, np.int64)
    ...
    def _spawn_worlds(self, idx):
        ...
        for n in idx:
            ...
            # ★ 지형 로테이션: 에피소드마다 사이트를 새로 뽑는다. 사이트가 1개면 항상 0.
            #   포메이션 로테이션과 **독립**으로 뽑아 (지형 × 포메이션) 조합이 다 나오게 한다.
            s = int(self.rng.integers(self.S)) if self.S > 1 else 0
            self.world_site[n] = s
            has_land_n = bool(self.site_has_land[s])
    ...
    def _reset_plans(self, idx):
        ...
        if getattr(self, "_committed", None) is not None:
            self._committed[idx] = False     # ★ 커밋 리셋(에피소드 시작)
    ...
    def reset(self, seed: int = None):
        ...
        # ★ 요격점 갱신형 커밋: 커밋 상태·커밋시 route·net_mask·요격점(재앵커 기준)
        self._committed = np.zeros((N, P), bool)
        self._commit_route = np.zeros((N, P, self.Kw, 2))
        self._commit_net = np.zeros((N, P, self.Kw), bool)
        self._commit_I = np.zeros((N, P, 2))
        # 격자·시간
    ...
    def _apply_actions(self, actions, force_fresh=None):
        cfg = self.cfg
        # ── ★ CNN 점수맵 모드: 선택 픽셀 → 절대 route/net_mask (셀 모드와 동형, 후보 리스트만 없음) ──
        if getattr(cfg, "cnn_action", False) and actions.get("pix") is not None:
            self._compute_assignment(actions.get("assign_pref"))    # obs/렌더 일관용(배정선)
            self._apply_cnn_actions(actions["pix"], force_fresh, actions.get("offset"))
            return
    ...
    def _mother_keepout(self, target):
        """★ 모선-전용 회피(APF와 별개). 추종 target 을 모선 죽음원반에서 바깥으로만 민다.

        아군-아군 척력은 없음(순수 RL 경로 유지) → 셀/CNN 정책으로 전이할 때 경로가 모선
        중심을 관통해 obstacle_collision 으로 격침되는 것을 방지(특히 wave).
        `cfg.mother_keepout=True` 일 때만 동작하며, `avoid_steer` 가 켜져 있으면 그쪽이 이긴다
        (전체 APF 가 모선 척력을 이미 포함하므로).

        ※ MobRobGPT `commander/cell_bridge.py` 가 의존하는 기능. 상류(One-Way_Towing/CNN)에는
          없으므로 env 이식 시 함께 옮긴다. 기본값 False 라 CNN 정책 경로에는 영향이 없다.
        """
        cfg = self.cfg; rc = self.rcfg; av = self.a_alive; c = self.center
        p = self.a_pos
        mx = p[..., 0] - c[0]; my = p[..., 1] - c[1]
        md = np.hypot(mx, my); dm = md - cfg.ally_mother_radius       # 죽음원반 가장자리까지
        R = float(getattr(rc, "mother_avoid_r", 500.0))               # APF 와 동일 세기
    ...
    def _micro(self, ev):
        ...
        evade = (self._enemy_evade() + self._enemy_land_avoid()).reshape(-1)
        ...
        if cap.any():
            # ★ 조기 제압 계측: 포획 시점의 '모선으로부터의 거리'를 누적한다.
            #   멀리서 잡을수록 조기 제압 — 포획 '수'만으로는 이 차이가 안 보인다.
            #   e_alive 를 끄기 전에 재야 한다(끄면 위치가 의미를 잃는다).
            cd = np.hypot(self.e_pos[..., 0] - self.center[0],
        ...
        # ── 충돌 → 큰 패널티 + 비활성화 ──
        track_dmin = bool(getattr(cfg, "dmin_track", False)) and ("mother_dmin" in ev)
        ...
        if P >= 2:                                       # 아군-아군 (둘 다 비활성화)
            ...
            for a in range(P):
                for b in range(a + 1, P):
                    ...
                    if track_dmin:      # ★ 죽기 전 거리 — 양쪽 배 모두에 기록
                        pair = self.a_alive[:, a] & self.a_alive[:, b]
                        dpair = np.where(pair, dd, np.inf)
                        ev["ally_dmin"][:, a] = np.minimum(ev["ally_dmin"][:, a], dpair)
                        ev["ally_dmin"][:, b] = np.minimum(ev["ally_dmin"][:, b], dpair)
        # 아군-모선(항공모함) 충돌
        md = np.hypot(self.a_pos[..., 0] - self.center[0],
                      self.a_pos[..., 1] - self.center[1])
        if track_dmin:              # ★ a_alive 를 '죽기 전' 위치에 걸어야 충돌 배가 기록된다
            ev["mother_dmin"] = np.where(self.a_alive,
                                         np.minimum(ev["mother_dmin"], md), ev["mother_dmin"])
        ...
        # ── ★ 아군-육지 충돌 → 비활+패널티 ──────────────────────────────
        #   육지는 그동안 적 APF·행동마스크·관측에만 쓰였고 아군은 섬을 그대로 통과했다.
        #   배가 섬에 얹히는 것은 모선 충돌과 같은 급의 손실이므로 동일하게 비활성화한다.
        #   ★ 지형 로테이션 이후 land_grid 는 [S,G,G] 이므로 월드의 사이트축을 함께 인덱싱한다.
        if self.has_land and bool(getattr(cfg, "ally_land_collision", True)):
            on_land = self.land_grid[self.world_site[:, None], aci, acj] & self.a_alive  # [N,P]
            ev["land_collisions"] += on_land.sum(axis=1)
            newly_dead |= on_land
    ...
    def fresh_ev(self):
        ...
        #   라는 설계 의도가 정작 충돌 케이스에서만 소거됐다. 여기서는 죽기 전 위치를
        #   매 스텝 기록해 그 구멍을 메운다.
        ev["mother_dmin"] = np.full((self.N, self.P), np.inf)
        ev["ally_dmin"] = np.full((self.N, self.P), np.inf)
    ...
    def _push_off_land(self, pts, site=0):
        """육지 위 스폰점 [K,2] 을 물로 옮긴다 — 모선기준 **방위를 회전**시켜 찾는다.

        반경을 유지해야 스폰 난이도(거리)가 보존되므로 반경이 아니라 방위를 돌린다.
        ±2°씩 좌우 번갈아 최대 ±60° 탐색, 그래도 물이 없으면 원위치(그 방위는 통째로 육지).
        `site` = 이 월드가 배정된 해역 인덱스(지형 로테이션).
        """
        lg = self.land_grid[int(site)]
        p = np.asarray(pts, np.float64).copy()
        c = self.center
        rel = p - c
        r = np.hypot(rel[:, 0], rel[:, 1])
        b = np.degrees(np.arctan2(rel[:, 0], rel[:, 1]))
        for k in range(1, 31):
            ci = np.clip((p[:, 0] / self.cell).astype(np.int64), 0, self.G - 1)
            cj = np.clip((p[:, 1] / self.cell).astype(np.int64), 0, self.G - 1)
            bad = lg[ci, cj]
            if not bad.any():
                break
            d = (2.0 * k) * (1 if k % 2 else -1)             # +2, −4, +6, −8 …
            nb = np.deg2rad(b + d)
            cand = c + np.stack([np.sin(nb), np.cos(nb)], -1) * r[:, None]
            p = np.where(bad[:, None], cand, p)
        return p

    def _enemy_land_avoid(self):
        """★ 적 육지 APF 우회 [N,M] (deg). 좌초가 아니라 '돌아간다'.

        모선 인력(단위벡터) + 육지 척력(거리에 따라 감쇠)을 합성해 **원하는 방위**를 만들고,
        모선 직행 방위와의 차이를 조향각으로 돌려준다. `enemy_step` 이 이 값을 bearing 에
        더하므로 그물 evade 와 같은 통로를 쓴다(둘은 합산된다).

        척력장은 정적이라 __init__ 에서 1회 계산됐다 → 여기선 O(1) 격자 조회뿐.
        척력이 인력과 정확히 반대인 정면충돌 각에서는 합이 0이 되어 갇힐 수 있으므로,
        그 경우 접선 방향으로 밀어 데드락을 푼다."""
        cfg = self.cfg
        if self.land_dist is None or not self.e_alive.any():
            return np.zeros((self.N, self.M))
        c = self.center
        ex, ey = self.e_pos[..., 0], self.e_pos[..., 1]              # [N,M]
        ci = np.clip((ex / self.cell).astype(np.int64), 0, self.G - 1)
        cj = np.clip((ey / self.cell).astype(np.int64), 0, self.G - 1)
        ws = self.world_site[:, None]                                # [N,1] 사이트축 인덱스
        d = self.land_dist[ws, ci, cj]                               # [N,M] 최근접 육지 거리(m)
        away = self.land_away[ws, ci, cj]                            # [N,M,2] 멀어지는 방향

        R = float(getattr(cfg, "enemy_land_range", 900.0))
        w = np.clip(1.0 - d / max(R, 1e-6), 0.0, 1.0)                # 가까울수록 1
        w = w * w                                                    # 부드러운 감쇠(2차)
        gain = float(getattr(cfg, "enemy_land_gain", 1.6))
        rep = away * (gain * w)[..., None]                           # 반경 척력

        gx, gy = c[0] - ex, c[1] - ey                                # 모선 인력
        gn = np.maximum(np.hypot(gx, gy), 1e-6)
        att = np.stack([gx / gn, gy / gn], axis=-1)

        # ★ 접선(vortex) 성분 — 반경 척력만 쓰면 '섬 정면돌파' 각에서 인력과 상쇄돼
        #   국소최소에 갇힌다(실측: 육지 근방 적의 24.8 % 가 이 각도). 척력에 수직인 성분을
        #   더해 **밀어내는 대신 돌아가게** 만든다. 부호는 현재 진행방향과 같은 쪽을 골라
        #   (되돌아가지 않게) 자연스러운 우회가 되도록 한다.
        tang = np.stack([-away[..., 1], away[..., 0]], axis=-1)      # away 를 +90° 회전
        hv = K.heading_vec(self.e_hdg)                               # [N,M,2] 현재 진행방향
        side = np.sign((tang * hv).sum(-1))                          # 진행방향과 같은 쪽
        side = np.where(side == 0.0, 1.0, side)
        head_on = np.clip((att * (-away)).sum(-1), 0.0, 1.0)         # 1 = 육지 정면
        tw = float(getattr(cfg, "enemy_land_tangent", 1.8)) * w * head_on
        v = att + rep + tang * (side * tw)[..., None]

        vn = np.hypot(v[..., 0], v[..., 1])
        dead = (vn < 1e-3) & (w > 0)                                 # 그래도 상쇄되면 순수 접선
        if dead.any():
            v = np.where(dead[..., None], tang * side[..., None], v)

        want = np.degrees(np.arctan2(v[..., 0], v[..., 1]))          # 원하는 방위
        base = np.degrees(np.arctan2(gx, gy))                        # 모선 직행 방위
        turn = (want - base + 180.0) % 360.0 - 180.0                 # wrap180
        return np.where(w > 0, turn, 0.0) * self.e_alive
    ...
    def _raycast_cover_detail(self):
        ...
        c = np.asarray(self.center)
        # ★ raycov_plan_path: 접근세그먼트(a_pos→첫WP) 포함 + 모든 계획 leg 인정(그물벽만이 아님)
        if getattr(self.cfg, "raycov_plan_path", False):
            aug = np.concatenate([self.a_pos[:, :, None, :], self.route], axis=2)   # [N,P,Kw+1,2]
            r0 = aug[:, :, :-1, :]; r1 = aug[:, :, 1:, :]                           # [N,P,Kw,2]
            leg_net = np.ones((N, P, r0.shape[2]), bool)                            # 모든 계획 leg
        else:
        ...
        t_block = np.where(blk, tmin, np.inf).min(2)       # [N,M] 가장 이른(모선서 먼) 차단 t (inf=계획차단없음)
    ...
    def _raycast_single_cover_vec(self):
        ...
        **충돌거리만큼 가까이** 같은 ray 를 막으면 redundant→무효(0): 경로 겹침=충돌 예정.
        ...
        if not self.e_alive.any():
            return np.zeros(self.N)
        nb, collide, al, t_block = self._raycast_cover_detail()   # 계획 net-leg(배별·충돌판정·차단t)
        net_painted = self._ray_net_blocked()              # 실제 설치 그물(persistent)
        covered = ((nb >= 1) | net_painted) & (~collide) & al   # 합집합 커버, 충돌근접만 무효
        # ★ 원거리 차단 가산: 적 ray 를 모선서 멀리(t→0=적 가까이) 막을수록 배율↑ (종심방어)
        gain = float(getattr(self.rcfg, "raycov_far_gain", 0.0))
        mult = np.where(np.isfinite(t_block), 1.0 + gain * np.clip(1.0 - t_block, 0.0, 1.0), 1.0)
        val = np.where(covered, self.rcfg.raycov_net_bonus * mult, 0.0)
    ...
    def _reward(self, ev, phi0, phi1, period):
        ...
        base = RW.window_reward_vec(
            ...
            land_collisions=ev["land_collisions"],
            cap_dist_mean_norm=(ev["cap_dist_sum"]
                                / np.maximum(ev["cap_dist_n"], 1.0)
                                / max(self.cfg.cell_r_max, 1.0)))
        ...
        # ── ★ 윈도우 최소거리 근접 페널티 — 위 척력의 사각을 메운다 ──────────
        #   위 mother_rep 은 pos_end × a_alive 라 **충돌한 배의 기여가 정확히 0** 이다
        #   (_micro 에서 a_alive 가 꺼지므로). 즉 정작 벌해야 할 케이스에서 dense 신호가
        #   사라지고 사후 r_obstacle 만 남는다. 여기서는 '죽기 전 최근접'을 쓴다.
        wmd = float(getattr(rc, "w_mother_dmin", 0.0))
        wad = float(getattr(rc, "w_ally_dmin", 0.0))
        if wmd > 0.0 and "mother_dmin" in ev:
            Rm = self.cfg.ally_mother_radius + rc.mother_avoid_r      # 영향 시작 반경
            dmin = ev["mother_dmin"]
            prox = np.clip((Rm - dmin) / max(rc.mother_avoid_r, 1e-6), 0.0, 1.0)
            prox = np.where(np.isfinite(dmin), prox, 0.0)             # inf=미측정 → 0
            obstacle = obstacle + wmd * (prox ** 2).sum(1)            # 제곱: 붙을수록 급가중
        if wad > 0.0 and "ally_dmin" in ev:
            dmin = ev["ally_dmin"]
            prox = np.clip((rc.avoid_r - dmin) / max(rc.avoid_r, 1e-6), 0.0, 1.0)
            prox = np.where(np.isfinite(dmin), prox, 0.0)
            obstacle = obstacle + wad * (prox ** 2).sum(1)
    ...
    def _compute_assignment(self, assign_pref=None):
        ...
        # ── ★ 잉여 전력 배정: 1:1 매칭 뒤 남은 배를 '가장 과부하인' 활성 클러스터에 덧붙인다 ──
        #   활성 클러스터가 아군 수보다 적으면(실측 평균 1.6개 / 아군 3척) 남는 배는
        #   _assign=-1 → route=a_pos 로 **영구 정지**했다. 정책이 그 배를 위해 찍은 픽셀도 버려진다.
        #   겹침 레짐(cnn_gate_disjoint=False)이 이미 다층 벽을 허용하고, raycov 는 서로 먼 층을
        #   redundant 로 치지 않으므로 같은 클러스터에 배를 더 붙이는 것이 설계와 일관된다.
        #   부하 = 적 수 /(1+배정수) 가 큰 클러스터부터, 그 교점에 가장 가까운 유휴 배를 붙인다.
        if bool(getattr(cfg, "assign_surplus", True)) and P > 1:
            nass = np.stack([(assign == k).sum(1) for k in range(K)], axis=1).astype(np.float64)
            for _ in range(P):
                free = self.a_alive & (assign < 0)                        # [N,P]
                if not free.any():
                    break
                load = np.where(active, cnt / (1.0 + nass), -1.0)         # [N,K]
                kb = np.argmax(load, axis=1)                              # [N]
                # ★ 같은 요격점에 겹쳐 붙이면 벽이 중복돼 오히려 나빠진다(concentrated 실측 -0.053).
                #   n 번째 덧붙는 배는 모선 쪽으로 한 층 안쪽(t 증가)에 요격점을 잡아
                #   **다층 방어(종심)** 를 만든다 — 겹침 레짐이 보상하는 바로 그 형태다.
                dt = float(getattr(cfg, "assign_layer_dt", 0.18))
                tl = np.clip(t + dt * nass[rows, kb], 0.05, 0.95)          # [N]
                cb = cent[rows, kb]                                        # [N,2] 클러스터 중심
                Ib = cb + tl[:, None] * (c[None, :] - cb)                  # [N,2] 층별 요격점
                # ★ 방위로도 벌린다: 반경만 나누면 두 배가 **같은 방위선** 위를 앞뒤로 달려
                #   경로가 겹치고 아군충돌이 는다(실측 conc 0→9). 층마다 ±dbear 로 번갈아
                #   틀어 옆으로 떼어놓는다. 모선 기준 회전이라 요격 환형 위를 미끄러진다.
                dbear = float(getattr(cfg, "assign_layer_dbear", 0.0))
                if dbear != 0.0:
                    j = nass[rows, kb]                                     # 이 클러스터의 기존 배정수
                    sgn = 1.0 - 2.0 * (j.astype(np.int64) % 2)             # +1,-1,+1,...
                    ang = np.deg2rad(dbear * sgn * ((j.astype(np.int64) + 1) // 2))
                    rel = Ib - c[None, :]
                    ca, sa = np.cos(ang), np.sin(ang)
                    Ib = c[None, :] + np.stack(
                        [rel[:, 0] * ca - rel[:, 1] * sa,
                         rel[:, 0] * sa + rel[:, 1] * ca], axis=-1)
                d = np.hypot(self.a_pos[..., 0] - Ib[:, 0:1],
                             self.a_pos[..., 1] - Ib[:, 1:2])             # [N,P]
                d = np.where(free, d, BIG)
                pb = np.argmin(d, axis=1)                                 # [N]
                sel = np.where((load[rows, kb] > 0.0) & (d[rows, pb] < BIG / 2))[0]
                if len(sel) == 0:
                    break
                assign[sel, pb[sel]] = kb[sel]
                assignI[sel, pb[sel]] = Ib[sel]          # 층별 요격점(1:1 배정은 원래 t 유지)
                nass[sel, kb[sel]] += 1.0
    ...
    def step(self, actions):
        ...
        info = {"captures": ev["captures"], "breaches": ev["breaches"],
                ...
                "land_collisions": ev["land_collisions"],       # [N] 아군-육지 충돌(비활성화)
                "cap_dist_sum": ev["cap_dist_sum"],             # [N] 포획거리 합(m) — 조기제압
                "cap_dist_n": ev["cap_dist_n"],                 # [N] 포획 건수(평균 분모)
    ...
    def _apply_cell_actions(self, cells, force_fresh=None, offset=None):
        ...
        # ★ 요격점 갱신형 커밋: 커밋한 배는 셀 재선택 무시, 벽을 '이동한 요격점' 따라 재앵커(추적)
        if getattr(self.cfg, "cell_commit", False):
            act = self.a_alive & (self._assign >= 0) & (self.a_nets > 0)   # 활동가능
            nm = net_mask.astype(bool)
            hold = self._committed & act                                   # 유지 커밋(전개전 추적)
            if hold.any():
                shift = (self._assignI - self._commit_I)[:, :, None, :]    # 요격점 이동량
                reanch = np.clip(self._commit_route + shift, 0.0, self.cfg.world_size)
                route = np.where(hold[..., None, None], reanch, route)
                nm = np.where(hold[..., None], self._commit_net, nm)
            newc = act & (~self._committed)                               # 새 커밋(이번 route 고정)
            if newc.any():
                self._commit_route = np.where(newc[..., None, None], route, self._commit_route)
                self._commit_net = np.where(newc[..., None], nm, self._commit_net)
                self._commit_I = np.where(newc[..., None], self._assignI, self._commit_I)
                self._committed = self._committed | newc
            self._committed = self._committed & act                        # 그물소진/미배정 → 해제
            net_mask = nm.astype(net_mask.dtype)
    ...
    # ── ★ CNN 점수맵 행동공간 (cnn_map.md) ──────────────────────────────
    #   셀 모드와 동형: '유효 마스크 안에서 K개 위치를 고르고 → build_routes_from_cells 로 벽 구성'.
    #   다른 점은 후보가 정적 리스트(C≈272)가 아니라 마스크된 점수맵 픽셀이라는 것뿐이다.

    def _cnn_valid_mask(self):
        """배별 후보 픽셀 유효마스크 [N,P,n,n] (True=선택 가능).
        환형 ∩ 요격점 반경게이트 ∩ 방위게이트 ∩ (선택) Voronoi 분할, 그 뒤 **하한 폴백**.
        기본 30² 저규제 레짐에서 배당 후보는 min≥cnn_min_valid(150) · median≈180 (환형 364/900).
        build_cnn_obs·heuristic_pix 가 **이 배열을 공유**해야 BC 타깃이 -inf 에 안 떨어진다."""
        from . import cnn_map as CM
        cfg = self.cfg; N, P = self.N, self.P
        n = CM.grid_n(cfg); c = self.center
        if not hasattr(self, "_assign"):                             # 첫 호출(리셋 직후) 안전장치
            self._compute_assignment()
        base = (CM.annulus_mask(cfg) if getattr(cfg, "cnn_mask_annulus", True)
                else np.ones((n, n), bool))                          # [n,n] 정적
        # ★ 육지 제외는 base 에 넣는다 — 폴백 사다리가 base 까지 되돌리므로, 여기 넣어야
        #   "후보가 모자라 폴백" 상황에서도 육지가 되살아나지 않는다(물리적으로 불가능한 선택).
        #   ★ 지형 로테이션: 육지는 월드마다 다르므로 base 가 [n,n] 이 아니라 [N,1,n,n] 이 된다.
        land_w = None                                                # [N,n,n] 또는 None
        if (self.has_land and getattr(cfg, "land_mask_action", True)
                and self.land_map.shape[1:] == (n, n)):
            land_w = self.land_map[self.world_site]                  # [N,n,n]
        XX, YY = CM.pixel_centers(cfg)
        assigned = self._assign >= 0                                 # [N,P]
        # 게이트 중심: 배정=요격점, 미배정=자기 위치(빈 마스크 방지. 어차피 아래서 정지 처리)
        Gc = np.where(assigned[..., None], self._assignI, self.a_pos)   # [N,P,2]
        d = np.hypot(XX[None, None] - Gc[..., 0, None, None],
                     YY[None, None] - Gc[..., 1, None, None])        # [N,P,n,n]
        base_w = base[None, None] if land_w is None else (base[None, None] & ~land_w[:, None])
        base_b = np.broadcast_to(base_w, (N, P, n, n))
        # ── 두 게이트를 **따로** 만든다. 폴백에서 무엇을 먼저 버릴지가 의미를 좌우하기 때문이다.
        #   · 반경게이트 = 이동비용 예산 (요격점에서 얼마나 멀리 놓아도 되나)
        #   · 방위게이트 = **배정 클러스터로 뻗는 코리도** — 이게 '무엇을 막는 그물인가'를 정한다
        #   방위를 잃으면 배정과 무관한 방향에 그물을 놓게 되므로, 폴백은 반경부터 푼다.
        gr = float(getattr(cfg, "cnn_gate_r", 0.0))
        r_ok = (d <= gr) if gr > 0.0 else np.ones((1, 1, 1, 1), bool)
        amax = float(getattr(cfg, "cnn_gate_angle", 180.0))
        if amax < 180.0:
            _, PB = CM.pixel_polar(cfg)                              # 픽셀 방위 [n,n]
            # Gb = 요격점 방위. I = cent + t·(center−cent) 이므로 **정확히 클러스터 방위**와 같다
            # (실측 오차 0.00°). 즉 부채꼴의 중심선은 '모선→배정 클러스터' 축이다.
            Gb = np.degrees(np.arctan2(Gc[..., 0] - c[0], Gc[..., 1] - c[1]))   # [N,P]
            db = np.abs(((PB[None, None] - Gb[..., None, None] + 180.0) % 360.0) - 180.0)
            a_ok = db <= amax
        else:
            a_ok = np.ones((1, 1, 1, 1), bool)
        gate = base_b & r_ok & a_ok
        gate_keep_angle = base_b & a_ok      # ★반경만 푼 것 — 코리도 방향은 유지 (폴백 1순위)
        valid = gate
        # ★ Voronoi 분할: 픽셀을 최근접 '배정된' 배에만 → 배별 세트 disjoint → WP 겹침 불가
        if getattr(cfg, "cnn_gate_disjoint", True) and P > 1:
            comp = np.where(assigned[..., None, None], d, np.inf)
            best = np.argmin(comp, axis=1)                           # [N,n,n] 픽셀별 최근접 배정배
            own = best[:, None] == np.arange(P)[None, :, None, None]
            any_asg = assigned.any(1)[:, None, None, None]
            valid = valid & np.where(assigned[..., None, None] & any_asg, own, True)
        # ★ 폴백 사다리 — 하한이 **두 종류**다. 이걸 하나로 합치면 '후보 수를 채우려고 방향을
        #   포기' 하게 된다(실측: ±60° 코리도가 하한 150 에 걸려 환형 전체로 폴백 → 반대편 50%).
        #     · soft = cnn_min_valid (예산): **반경만** 풀어서 채운다. 코리도 방향은 절대 안 버린다.
        #     · hard = cnn_nets       (크래시 방지): 코리도 전체가 K 픽셀도 안 될 때만 방향 포기.
        #   즉 코리도가 좁아 후보가 적으면 **적은 채로 둔다** — 엉뚱한 방향을 여는 것보다 낫다.
        hard = int(getattr(cfg, "cnn_nets", 2))
        soft = max(hard, int(getattr(cfg, "cnn_min_valid", 0)))
        # 최종 단(맵 전체)도 육지는 뺀다 — 물리적으로 못 놓는 곳은 어떤 폴백에서도 후보가 아니다
        allpix = (np.ones((1, 1, n, n), bool) if land_w is None
                  else ~land_w[:, None])                             # [1,1,n,n] | [N,1,n,n]
        for fb in (gate, gate_keep_angle):          # soft: 방향 보존 (Voronoi → 반경 순으로 해제)
            short = valid.sum((2, 3)) < soft
            if not short.any():
                break
            valid = np.where(short[..., None, None], fb, valid)
        for fb in (base_b, allpix):                 # hard: 여기서만 방향을 포기한다
            short = valid.sum((2, 3)) < hard
            if not short.any():
                break
            valid = np.where(short[..., None, None], fb, valid)
        return valid
    ...
    def build_cnn_obs(self):
        """CNN 점수맵 관측 {gmap, smap, own, valid} — obs/rasterizer.py 위임."""
        from .rasterizer import build_cnn_obs as _b
        return _b(self)

    def pix_to_routes(self, pix, offset=None):
        """선택 픽셀 flat idx [N,P,K] (+ 서브픽셀 offset[N,P,K,2]∈[-1,1])
        → world 점 [N,P,K,2] → **기존 build_routes_from_cells 재사용** → route/net_mask."""
        from . import cnn_map as CM
        from .cell_action import build_routes_from_cells
        n = CM.grid_n(self.cfg)
        f = np.clip(np.asarray(pix, np.int64), 0, n * n - 1)
        off = (np.asarray(offset, np.float64)
               if (offset is not None and getattr(self.cfg, "cnn_subpixel", True)) else None)
        pts = np.clip(CM.flat_to_world(self.cfg, f, off), 0.0, self.cfg.world_size)
        return build_routes_from_cells(pts, self.a_pos, self.a_alive,
                                       self.center, self.cfg, self.Kw)

    def _apply_cnn_actions(self, pix, force_fresh=None, offset=None):
        """선택 픽셀 → route/net_mask 절대 세팅 (매 결정 재계획, 설치중 그물 보호).
        force_fresh 는 셀 모드와 동일하게 인터페이스 호환용(픽셀=경로라 강제 커밋 불요)."""
        N, P, Kw = self.N, self.P, self.Kw
        route, net_mask = self.pix_to_routes(pix, offset)
        # ★ 미할당 배(assign<0)·그물소진 배는 정지 — 배정된 배만 WP 생성/그물
        done = (self.a_nets <= 0) | (self._assign < 0)
        if done.any():
            hold = np.broadcast_to(self.a_pos[:, :, None, :], (N, P, Kw, 2))
            route = np.where(done[..., None, None], hold, route)
        ...
        self._resid_mag = np.zeros((N, P))
        ptr_c = np.clip(self.ptr, 0, Kw - 1)
        self.net_end = np.take_along_axis(self.route, ptr_c[:, :, None, None],
                                          axis=2)[:, :, 0, :].copy()
        self.net_start = self.a_pos.copy()

    def heuristic_pix(self):
        """휴리스틱 배정 요격점서 코리도 가로 K점 → **유효 픽셀 내** 최근접 flat idx [N,P,K].
        heuristic_cells 와 같은 기하. 정책과 동일한 마스크·dup 반경을 써야 BC logp 가 유한하다."""
        from . import cnn_map as CM
        cfg = self.cfg; N, P = self.N, self.P; K = int(cfg.cnn_nets)
        self._compute_assignment()
        c = self.center; I = self._assignI
        rel = I - c[None, None, :]
        D = np.hypot(rel[..., 0], rel[..., 1])
        rad = rel / np.maximum(D, 1e-6)[..., None]
        perp = np.stack([-rad[..., 1], rad[..., 0]], axis=-1)        # 코리도 수직
        XX, YY = CM.pixel_centers(cfg)
        base = self._cnn_valid_mask()
        avail = base.copy()
        out = np.zeros((N, P, K), np.int64)
        for k in range(K):
            off = (k - (K - 1) / 2.0) * cfg.net_max_len              # 벽 양끝점
            pt = I + perp * off                                      # [N,P,2]
            d2 = ((XX[None, None] - pt[..., 0, None, None]) ** 2
                  + (YY[None, None] - pt[..., 1, None, None]) ** 2)
            d2 = np.where(avail, d2, np.inf)
            flat = d2.reshape(N, P, -1).argmin(-1)                   # [N,P]
            out[:, :, k] = flat
            ix, iy = CM.flat_to_ij(cfg, flat)
            avail = CM.disk_exclude(cfg, avail, ix, iy, cfg.cnn_dup_r)
            # ★ 도달성 상한 — 정책(_exclude)과 **같은 규칙**이어야 BC 타깃이 -inf 에 안 떨어진다.
            #   휴리스틱은 요격점 양옆 ±net_max_len/2 를 노리지만 '유효 픽셀 최근접' 스냅에서
            #   간격이 벌어진다(수정 전 실측 중앙 504 m > 450 m, 초과율 66.7 %).
            if bool(getattr(cfg, "cnn_reach_mask", False)):
                reach = cfg.net_max_len * float(getattr(cfg, "cnn_reach_slack", 1.0))
                avail = CM.reach_keep(cfg, avail, ix, iy, reach)
            still = avail.any((2, 3))                                # 다 막힌 배는 원복
            if not still.all():
                avail = np.where(still[..., None, None], avail, base)
        return out
