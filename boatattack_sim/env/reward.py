...
def window_reward(events: dict,
    ...
    r = 0.0
    # ── Unity 패리티 이벤트 ──
    #  포획 보상에 거리 배율(조기 제압). w_cap_dist=0 이면 정확히 기존 값.
    _cd = e.get("cap_dist_mean_norm", 0.0)
    r += cfg.r_capture * e.get("captures", 0) * (1.0 + cfg.w_cap_dist * _cd)
    ...
    r += cfg.r_land           * e.get("land_collisions", 0)
...
def window_reward_vec(captures, breaches, ally_collisions, nets_used,
                      ...
                      *, steps: int = 25, nets_total: int = 9,
                      obstacle_collisions=0.0, land_collisions=0.0,
                      cap_dist_mean_norm=0.0):
    ...
    landc = np.asarray(land_collisions, np.float64)
    ...
    cdn = np.asarray(cap_dist_mean_norm, np.float64)   # 0~1 (r_max 정규화)

    #  ★ 조기 제압 배율: 멀리서 잡을수록 포획 보상↑. w_cap_dist=0 이면 기존과 bitwise 동일.
    r = (cfg.r_capture * captures * (1.0 + cfg.w_cap_dist * cdn)
         + cfg.r_breach * breaches
