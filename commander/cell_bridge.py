...
class CommandedCellEnv(CommandedDefenseEnv):
    ...
    # ── 셀 격자·관측·행동적용은 **env 정식 구현을 그대로 쓴다** ──
    #   과거 이 클래스는 cell_world / n_cells / _cell_half / build_cell_obs / _apply_actions 를
    #   자체 구현했다. boatattack_sim/env 가 깨진 심볼릭 링크였던 시절의 임시방편이며, 격자가
    #   학습 원본과 어긋났다(cell_spacing 기반 280칸 vs make_cells 의 r_max 기반 272칸).
    #   env 를 실파일로 이식한 뒤에는 DefenseVecEnv 의 make_cells / build_cell_obs /
    #   _apply_actions 가 학습 코드 그대로이므로 그쪽이 정답이다. (재구현 제거: 2026-08-20)
